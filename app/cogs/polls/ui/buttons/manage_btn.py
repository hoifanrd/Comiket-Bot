...
class ManageButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        creator_id = database.get_poll_creator(self.poll_id)
        ...
        item_list = database.get_items_by_poll(self.poll_id)
        if not item_list:
            await interaction.response.send_message(
                "❌ 此投票沒有可管理的項目",
                ephemeral=True
        ...
        async def edit_callback(interaction: discord.Interaction):
            ...
            if modal.new_name or modal.new_price is not None:
                ...
                # 更新數據庫
                database.update_poll_item(self.poll_id, edit_item, new_name, new_price)
                # 重新載入投票消息
        ...
        async def delete_callback(interaction: discord.Interaction):
            ...
            async def confirm_delete(interaction: discord.Interaction):
                # 驗證投票是否仍然有效
                if database.get_poll_status(self.poll_id) == 'Ended':
                ...
                # 刪除項目
                database.delete_item_from_poll(self.poll_id, delete_item)
                # 重新載入投票消息
                await handler.update_poll_message(interaction.channel, self.poll_id)
