...
class MagicCommand:
    ...
    async def on_execute(self, interaction: Interaction, url: str):
        ...
        circle_data.social_link = utils.SOCIAL_LINK_PREFIX[link_domain] + name
    ...
    async def _create_circle_final_confirm(self, interaction: Interaction, circle_data: utils.CircleForm):
        ...
        async def confirm_callback(new_interaction: discord.Interaction):
            ...
            if found_circle_data is not None:
                found_channel = self.cog_twitter.bot.get_channel(found_circle_data.channel_id)
                if found_channel is not None:
                    channel_url = found_channel.jump_url
                    await interaction.edit_original_response(content=f"📊 品書已存在！\n{channel_url}", embed=None, view=None)
                    return
                else:
            ...
            if circle_data.shinagaki_img_urls:
                ...
                async def confirm_btn_callback(new_interaction: discord.Interaction):
                    ...
                    files = await self._generate_shinagaki_images(circle_data.get_link_domain(), circle_data.shinagaki_img_urls)
                ...
                await interaction.edit_original_response(content=f"🖼️偵測到 {circle_data.get_link_domain()} 推文中附有圖片、要把圖片傳到品書串嗎？", view=view, embed=None)
    ...
    async def _create_shinagaki_thread(self, interaction: discord.Interaction, circle_data: utils.CircleForm, files: list[discord.File] = None) -> discord.Thread:
        ...
        # Set space category of circle
        circle_data.space_cat = database.get_space_cat(circle_data)

        msg_title = utils.gen_thread_title(circle_data)
        thread_msg = utils.gen_thread_msg(circle_data)

        cur_day_channels = utils.DAY1_CHANNELS if circle_data.day == 1 else utils.DAY2_CHANNELS

        sent_thread_msg = await self.cog_twitter.bot.get_channel(cur_day_channels[circle_data.hall]) \
        ...
        await sql_database.add_circle(circle_data, new_thread.id)
        ...
        cog_poll = self.cog_twitter.bot.get_cog('Polls')
